...
# else, create vizualuzations 
  ...
  rcu_points = alt.Chart(source_on_map).mark_circle().encode(
      ...
      color=alt.Color('players_num_bucket', scale=alt.Scale(scheme='goldorange'), title ='Number of Players', 
                      legend = alt.Legend(orient = 'bottom-left',labelFontSize=14,titleFontSize=16,
                                          legendX=0, legendY=100)),
  ...
  rcl_points = rcl_base.mark_point(filled=True, size=300).encode(
      y=alt.Y('mean(dribbling):Q',scale=alt.Scale(domain=[0.5,1]),axis=alt.Axis(title='Normalized Mean Dribbling Score',titleFontSize=16,labelFontSize=12)),
